...
export function IndicatorToolListPage({ def }: { def: IndicatorToolUi }) {
  const auth = useAuth();
  ...
  const handleOpenEvaluation = async (
    ...
    try {
      ...
      const evaluation = await def.api.createEvaluation(
        org,
        { profileId },
        refreshedToken
      );
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <PageTitle rawTitle={t(`${def.i18n}.title`)} />
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            onClick={() => void navigate({ to: def.routes.admin })}
          >
          ...
          <Button variant="outline">
            ...
            {t("app.assessment.panel.exportReport")}
          </Button>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/60">
              <tr>
                ...
                {Array.from({ length: def.sectionCount }, (_, i) => i + 1).map(
                  (n) => (
                    <th scope="col" key={n} className="text-center py-3 px-2">
                      {t(`${def.i18n}.section`)} {n}
                    </th>
                  )
                )}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {row.isConsolidated && (
                          <span
                            ...
                            title={t(`${def.i18n}.consolidatedTooltip`)}
                          >
                            {t(`${def.i18n}.consolidatedLabel`)}
                          </span>
                        )}
                      </div>
                          ...
                          title={
                            canNavigateEvaluation
                              ...
                              : canNavigateAssociation
                                ? t(`${def.i18n}.associationViewLabel`)
                                : undefined
                          }
                          onClick={() => {
                        ...
                        {row.profile.type === "ASSOCIATION" &&
                          row.profile.associationLevel === "LEVEL_2" && (
                            <Button
                              variant="ghost"
                              size="icon"
                              title={t(`${def.i18n}.associationViewLabel`)}
                              onClick={() =>
                                void navigate({
                                  to: "/assessments/associations/$profileId",
                                  params: { profileId: row.profile.id },
      ...
      {distributionTotal > 0 && (
        <Card>
          <CardContent className="py-4 space-y-3">
            ...
            {[
              {
                label: t(`${def.i18n}.capacityHigh`),
                count: distribution.good,
                color: "var(--color-success)",
              },
              {
                label: t(`${def.i18n}.capacityMedium`),
                count: distribution.medium,
                color: "var(--color-warning)",
              },
              {
                label: t(`${def.i18n}.capacityLow`),
